...
def train(cfg, trainset, valset, testset):
    ...
    importance = compute_importance(model, testset, cfg["ih_batch_size"])
    raw, norm = importance_to_dict(importance, cfg["num_layers"])
    ih_data = {"dataset": cfg["dataset"], "model": "vanilla",
...
def main():
    ...
    CFG["ckpt_path"]    = os.path.join(ckpt_dir, f"{ds}_vanilla.pt")
    CFG["history_path"] = os.path.join(results_dir, f"{ds}_vanilla_history.json")
    CFG["ih_path"]      = os.path.join(results_dir, f"{ds}_vanilla_importance.json")
